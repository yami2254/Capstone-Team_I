...
public class CalendarMonthAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (  curDay + 7 <=  Calendar.getInstance().getActualMaximum(Calendar.DATE)) {
            if (position == getSelectedPosition() && saveidx  <= getSelectedPosition() && saveidx + 8 > getSelectedPosition() && getCurMonth() == realmon - 1) {

                Log.d("현재 달은", String.valueOf(Calendar.getInstance().getActualMaximum(Calendar.DATE)));
        ...
        {
            if (position == getSelectedPosition() && saveidx - 1 <= getSelectedPosition() && saveidx + lastindx > getSelectedPosition() && getCurMonth() == realmon - 1) {

                Log.d("현재 달은", String.valueOf(lastindx));
                itemView.setBackgroundColor(Color.YELLOW);
            } else if (position == getSelectedPosition() && lastindx < 7 && getCurMonth() == realmon && startidx - 1 <= getSelectedPosition() && startidx + (7 - lastindx - 3) > getSelectedPosition()) {
                Log.d("넘어간 달 ", String.valueOf(lastindx));
                itemView.setBackgroundColor(Color.YELLOW);
